...
def scan_heartbeat(
    ...
    with openDwfDevice(
        ...
        print("HELLO LOOK AT ME 1")


        _configure_analog_out_no_flash(
...
def _configure_analog_out_no_flash(
    ...
    analog_out.nodeEnableSet(channel_index, node, False)
...
def _calc_lockin(
    ...
    record_dc_lockin = np.average(record_demod)
    ##
...
def _demod(
    ...
    # if type(phase_demod) != int or type(phase_demod) != float:

    #     print("WARNING: phase demod is NAN")
    ...
    ##
    record_demod = record[:, 0] * ref_demod # is this correct >
    ##
...
def _plot_fft(
    ...
    ## Replace `None` with your code.
    ##
    fft_magnitudes = np.abs(np.fft.fft(trace))
    fft_frequencies = np.fft.fftfreq(len(trace), d=1/sample_frequency)

    are_geq_zero = (fft_frequencies > 0)
    fft_magnitudes_geq_zero = fft_magnitudes[are_geq_zero]
    fft_frequencies_geq_zero = fft_frequencies[are_geq_zero]
    ##
